...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      <SiteHeader />
      <main id="main">
        <header className="wrap project-head">
          <h1>{project.name}</h1>
          <hr className="masthead__rule" />
        </header>
        <div className="wrap project-lead">
        ...
        <div className="wrap story">
          {project.story.map((section, i) => (
            <section key={section.heading} aria-labelledby={`story-${i}`}>
              <h2 id={`story-${i}`}>{section.heading}</h2>
              <div>
        ...
        {project.notes.length > 0 && (
          <section className="wrap notes" aria-labelledby="notes-title">
            <h2 id="notes-title" className="section-title">Engineering notes</h2>
            <div className="story">
              {project.notes.map((section, i) => (
                <section key={section.heading} aria-labelledby={`note-${i}`}>
                  <h3 id={`note-${i}`}>{section.heading}</h3>
                  <div>
                    {section.body.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}
                  </div>
                </section>
