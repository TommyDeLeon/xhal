export type Project = {
  ...
  summary: string;
  /** Short paragraphs. Why it exists, what it does, and the hard part. */
  body: string[];
  ...
  liveUrl: string | null;
  /** Path under /public. Null renders the typographic fallback panel. */
  image: string | null;
...
export const projects: Project[] = [
  {
    ...
    body: [
      ...
      "The problem is chosen at the moment the lock fires rather than when the timer is armed, so it cannot be fetched and pre-solved in advance. Passing the tests is not enough on its own either. The submission also has to land inside a runtime budget, which means a correct but quadratic answer to a sliding-window problem still leaves you locked and tells you roughly how far off the pace it was.",
      "Making that rule fair turned out to be most of the work. Runtime budgets are stored per language, because a JVM cold start burns around 100ms before any user code runs while the equivalent C++ finishes in single digits, and one global number would make the gate unreachable in one language and free in another. Grading takes the fastest of several runs, since the judge measures wall-clock time on shared hardware that swings by tens of milliseconds and a single unlucky sample should not lock someone out of their own machine. A fixed millisecond floor sits underneath the percentage tolerance, because a 35% band around an 8ms target is narrower than the judge's own jitter.",
      "The lock is server-authoritative, which is the part I learned the most from. Clients never decide they are unlocked. The API signs a token bound to one user and session, issued only after a sandboxed judge reports a pass and the speed gate clears, and the desktop shell verifies that signature in a process the interface cannot reach. A patched web app, an injected script, or the console calling unlock with an empty string all stay locked.",
    ],
    stack: [
      "TypeScript",
      "Node 24",
