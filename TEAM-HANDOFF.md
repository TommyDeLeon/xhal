...
- **Favicon:** the lion on a transparent background, green in light browser themes and white in dark ones (the reverse tile was hard to see at 16px). Home-screen icons keep the reverse lion. Icon URLs use `?v=lion2` so browsers drop the cached "td" favicon. An audit found no "td" mark left in the source or the built site (only in the ignored `.next` cache).
