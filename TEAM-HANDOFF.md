...
Updated 25 September 2026 (second cloud session). Project: **xhal** (public brand: Tommy De Leon).

## Where things are

- **Branch:** `claude/tommydeleon-portfolio-rebuild-nvhjf2-fd7c57` (continues `claude/tommydeleon-portfolio-rebuild-nvhjf2`), built on `rebuild-8yc6ue` (all 17 desktop commits). The desktop WIP snapshot was folded into commit `Planning`. `main` is untouched at `bdbda36`.
...
- **Site status:** light-only rebuild done: home, `/work/<slug>/` for all three projects, the brand kit in `brand/`, favicons, the social preview and Playwright tests. **Only the CodeLock and Tenant101 media are missing.** Mimir now shows a real screenshot (`components/still.tsx`) instead of a film, with no play button; its poster is in `assets/posters/mimir.png` and `public/images/posters/mimir.*`.
...
## Second cloud session (desktop WIP folded in)

- Folded the desktop snapshot: Mimir uses a still instead of a film; updated CodeLock and Tenant101 film notes and credits; Mimir test count changed to 234.
- Ran `npm run images`, which generated `public/images/posters/mimir.{avif,webp,jpg}`.
- `npx tsc --noEmit`: pass. `npm run lint`: pass.
- `npm run build` with **temporary placeholder** CodeLock/Tenant101 media: pass (placeholders deleted, not committed). Without them, `check-assets` lists the 12 missing CodeLock/Tenant101 files.
- Playwright `chromium-desktop` + `chromium-mobile`: 18/18 pass, using the container's Chromium 1194 through a temporary config. The pinned Playwright expects a newer browser build that this container doesn't have.
- Checked Mimir screenshots on the home page and project page at 390px and 1440px.
- **Please check:** `content/projects.ts` says Mimir's hosted answers were "checked live on 26 September 2026". This session ran on 25 September 2026, so that date may be a typo. The 234-test count is also unverified here.
...
2. **Missing media:** add these files, then run `npm run build`:
   ...
   - Films: `public/films/<slug>/<slug>-landscape-720.mp4`, `<slug>-portrait-720.mp4` and `<slug>-descriptions.vtt`, for slugs `codelock` and `tenant101`. Mimir needs no film until one is finished.
3. **Claims to recheck by running the tests** (a quick count of the source doesn't match):
   ...
   - Mimir: the site now says 234 passing backend tests (changed on the desktop). Unverified from the cloud.
